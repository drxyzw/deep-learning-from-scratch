import numpy as np
from common.utils import softmax, cross_entropy_error, OUTPUT_DEBUG, im2col, col2im
...
class Convolutiono:
    ...
    def backward(self, dout):
        ...
        # self.db = np.sum(dout, axis=0) # this is wrong on their website
        self.db = dout
...
class Pooling:
    ...
    def backward(self, dout):
        ...
        # here, we set non-zero value only for max arg
        dmax[np.arange(self.arg_max.size), self.arg_max.flatten()] = dout.flatten()
        dmax = dmax.reshape(dout.shape + (pool_size,)) # adding a new dimension
