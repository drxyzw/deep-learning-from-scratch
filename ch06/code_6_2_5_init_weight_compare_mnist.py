...
if __name__ == "__main__":
    ...
    if isinstance(weight_init_std_type, numbers.Number):
        ...
        weight_init_std["W3"] = 1. / np.sqrt(hidden_size)
        weight_init_std["W4"] = 1. / np.sqrt(hidden_size)
        weight_init_std["W5"] = 1. / np.sqrt(hidden_size)
    elif weight_init_std_type.upper() == "HE":
        weight_init_std = {}
        weight_init_std["W1"] = np.sqrt(2. / input_size)
        weight_init_std["W2"] = np.sqrt(2. / hidden_size)
        weight_init_std["W3"] = np.sqrt(2. / hidden_size)
        weight_init_std["W4"] = np.sqrt(2. / hidden_size)
        weight_init_std["W5"] = np.sqrt(2. / hidden_size)
    ...
    df_output.to_excel(f"./ch06/five_learning_backward_{optimizer.__class__.__name__}_weight_init_{weight_init_std_type}.xlsx", index=True)
